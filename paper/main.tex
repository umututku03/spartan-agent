...
relative to holding ETH, without claiming any excess return. A follow-up places a language model in
the decision loop and executes its choices through the real agent on the fork; across three models
from two providers the behaviour is dominated by risk posture rather than model choice, reported as an
illustrative result rather than a performance claim. Measured against an external benchmark's own ETH
task, data, and metrics, our contamination-free policy lands among published LLM agents while a naive
price-only LLM does not.
\end{abstract}
...
\section{LLM-in-the-loop evaluation (follow-up)}\label{sec:llm}
As a follow-up we put a language model in the decision loop, in two stages, and we are careful to keep
the claims small.

\subsection{Deterministic policy comparison}
Using the same no-lookahead stepping, we let a model choose a target ETH exposure each day from only
past returns and trailing volatility (temperature 0, with decisions cached so runs are reproducible),
and compared three models across two providers (\texttt{gpt-4o-mini}, \texttt{gpt-4o},
\texttt{claude-haiku-4.5}). We report three arms: the model's exposure used directly
(\emph{llm-raw}), the model's exposure clamped to our volatility-targeted budget (\emph{llm+risk}), and
the deterministic baselines. Over the same bear year, all three models, prompted to be risk-aware,
stayed cautious and cut drawdown sharply relative to holding ETH (about 24\%, 28\%, and 30\% versus
67\%), with the model and the provider mattering little. This lines up with AI-Trader's observation
that risk control, not raw model strength, drives robustness~\cite{aitrader}. In this run the
deterministic guard did not bind (\emph{llm+risk} equalled \emph{llm-raw}) because the models never
asked for more than the budget the guard enforces; the guard is a backstop against oversizing, and its
binding case is the deterministic result in Section~\ref{sec:results}. Figure~\ref{fig:llm} shows the
drawdown and volatility by arm.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/llm_benchmark.pdf}
\caption{LLM-in-the-loop vs.\ deterministic sizing on ETH (full year). All three models, prompted to be
risk-aware, hold low exposure and so cut drawdown and volatility relative to buy-and-hold;
\emph{llm+risk} equals \emph{llm-raw} because the guard never binds here.}
\label{fig:llm}
\end{figure}

\subsection{Agent-in-the-loop proof of concept}
We then replayed the model's own cached decisions through the real deployed agent: each decision became
a chat command that the Spartan agent executed as an actual Uniswap V2 swap on the mainnet fork. Six
consecutive decisions produced six confirmed transactions (status success) from the imported wallet.
This closes the loop end to end, from a language-model decision to an on-chain action through the
deployed agent. It is a capability demonstration at a single forked block, not a historical backtest:
the fork does not advance through the six days, and the exposure-to-swap mapping is illustrative. The
two stages answer different questions, reproducible measurement and end-to-end execution; details are
in \texttt{docs/BENCHMARK\_LLM.md}.

\subsection{Comparison against an external benchmark}\label{sec:external}
The evaluations above use our own harness, so we also measured the same policy against a published
external benchmark. We chose InvestorBench~\cite{investorbench}, which includes an ETH-USD task with
open data and per-model results. Its full harness requires a multi-GPU serving stack, a vector
database, a memory agent, and an embedding model, so we did not run it. Instead we reproduced its ETH
task on its exact price series, test window (3 Apr to 5 Nov 2023, 186 daily returns), and metric
definitions, porting its formulas verbatim, and compared our policy to its published numbers. As a
check that the reproduction is faithful, buy-and-hold through our pipeline matches its reported ETH
buy-and-hold to three decimals (cumulative return 4.53\%, Sharpe 0.146, annualized volatility 41.82\%,
maximum drawdown 29.89\%) with nothing tuned to fit.

On this window our contamination-free volatility-targeted policy returns 2.28\% (Sharpe 0.08), between
the benchmark's GPT-4 (1.52\%) and buy-and-hold (4.53\%), and trims drawdown slightly below holding
ETH. The window was gently rising, so a policy whose job is to cut exposure when volatility spikes
gives up a little upside; that is the same guard whose payoff in a falling market appears in
Section~\ref{sec:llm}. A stripped-down variant, \texttt{gpt-4o-mini} making daily buy/sell/hold calls
from a price-only prompt, returned $-11.70\%$, close to the benchmark's weakest reported backbone
(Llama-3.1-70B at $-11.89\%$). We do not read that as a like-for-like model verdict: our LLM arm omits
the news and memory that the benchmark's agents receive, and the 2023 window predates the model's
training cutoff, so it is contamination-prone. The point is narrower and, we think, defensible: on an
external benchmark's own task, data, and metrics, a simple auditable sizing rule lands among published
LLM agents, while a naive directional LLM without a risk layer is fragile. The honesty ledger of what
matches and what differs is in \texttt{docs/BENCHMARK\_EXTERNAL.md}; Figure~\ref{fig:external} shows the
comparison.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/investorbench_eth.pdf}
\caption{InvestorBench ETH task (3 Apr to 5 Nov 2023): our arms (blue) against the benchmark's
published agents (gray). Our reproduced buy-and-hold matches their published value exactly, validating
the setup; our deterministic policy lands mid-pack, and a price-only LLM arm (starred, contamination-prone
and without the benchmark's news and memory) is among the weakest.}
\label{fig:external}
\end{figure}
...
Future work is the part we have not earned yet. Section~\ref{sec:external} compares our policy to a
published benchmark's ETH task on its own data and metrics, but we reproduced the protocol rather than
running the benchmark's full harness, and our LLM arm omits its news and memory. The next steps are a
multi-seed, multi-asset, out-of-sample evaluation inside that harness end to end; scaling the
LLM-in-the-loop evaluation of Section~\ref{sec:llm} (more assets, seeds, and providers); richer
